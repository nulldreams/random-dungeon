...
#		apply_movement(Vector2(1, 1) * movement.acceleration * delta)
...
func _on_detection_range_body_entered(body):
	if body.is_in_group("players"):
		if "goblin" in name:
			var phrases = ["Can you hack Facebook?", "Dude, come here!", "Yo!"]
			var rng = RandomNumberGenerator.new()
			rng.randomize()
			$"dialog/Label".text = phrases[rng.randi_range(0, phrases.size()-1)]
			$"dialog".visible = true
